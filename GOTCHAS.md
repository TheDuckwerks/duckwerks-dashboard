...
**2026-07-27: eBay put sold listings behind a login wall, so SerpAPI's `show_only=Sold` returns a successful empty result forever.** SerpAPI's scraper has no eBay session, so the sold filter serves it eBay's "Sign in to your account" page, which it reports as `"status": "Success"` with `"organic_results_state": "Fully empty"` and the message *"eBay hasn't returned any results for this query."* Confirmed in SerpAPI's own playground: the rendered HTML pane is literally the eBay login form. Same query unfiltered returns `total_results: 921` in 2.31s; with the sold filter it burns 50-90s retrying into the wall and returns nothing. A logged-in browser sees 877 sold results for that identical search, so the data exists and only the scraper is blocked.

*The tell:* `organic_results_state: "Fully empty"` plus a 50-90s `total_time_taken`. A healthy search answers in seconds. Slowness here is the failure mode, not load.

*What is NOT the cause, all of it checked on 2026-07-27:* the request params in `server/comps.js` are correct as written (`_ipg` and `show_only` were both suspected and cleared, though passing `_ipg` at all does throw a separate hard error and it buys nothing — the default page size is 60); the SerpAPI account was healthy at 998/1000 searches; the nginx vhost 504 at 60s was a downstream symptom of the slow failure, not a cause (filed as ops#115, corrected there and downgraded to hardening). Don't re-debug any of those.

**eBay's Marketplace Insights API is permanently off the table.** It is the official source for sold-listing data and it needs a separate gated approval. Geoff applied years ago, was refused, and will not re-apply. Do not propose it as the fix for comp research; the answer is already no. Combined with the 2026-03-29 entry above, every automated sold-comp path is now closed: native scrape blocked, Finding API not enabled, Browse API is active-listings only, Marketplace Insights denied, SerpAPI walled. Sold comps are a manual, logged-in-browser job until something upstream changes. Active listings via SerpAPI still work fine and fast, and are the available automated signal.

**2026-07-27: the manual sold pull is now a DOM extractor, not a text paste.** `.claude/skills/list-item/pull-sold-comps.js` reads the live `s-card` DOM in a logged-in eBay sold-results tab and emits the exact `listings[]` shape `/api/comps/analyze` consumes, so the analyze flow is unchanged from the SerpAPI days — only the source of the listings moved. Runs as the "Pull Sold Comps" bookmarklet or a console paste; copies JSON to the clipboard. Selectors are eBay's `s-card` markup (title `.s-card__title`, price `.s-card__price` distinct from the `.strikethrough` original, sold caption `.s-card__caption`); if eBay reworks them the pulled count drops or reads 0, which is the signal to re-pull one card's `outerHTML` and re-map. Verified live at build on two searches (52 and 60 rows, auction + BIN). list-item Phase 4 drives it; #168 closed resolved-by-workaround (the automated active-listings mode was not needed).

**2026-07-27: eBay pads a thin Sold results page with "related" sold items, and the extractor can't filter them.** When a Sold search returns fewer than a full page, eBay hydrates the rest of the page with related sold listings from adjacent categories. The extractor gates on a real "Sold" caption, and these ARE genuine sold items, just of the wrong part, so they pass the gate and land in the pull. Seen on the iMac cooling-parts pull: a `iMac A1418 fan` search came back with APA007 power supplies, logic boards, LCD screens, and whole machines mixed in. The tell is a pull far larger than the real pool with obviously off-part titles. Triage by title before pricing; for a thin-pool item with no direct comp, reading the relevant rows and setting a judgment number beats sending the polluted set to `/analyze`. Also: two extractor pulls pasted into one file are two arrays (`[...][...]`), invalid JSON — merge with `jq -s 'add'`.
