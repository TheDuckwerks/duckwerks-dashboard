...
- **better-sqlite3 is native.** `npm ci` on the box fetches a prebuilt binary for its Node version. A Node bump with no matching prebuilt falls back to compiling; the build toolchain is installed on the box as insurance, so that degrades to a slower deploy, not a broken one.
...
- The db is reachable from the Mac via `scripts/db.sh` (rides `duckops@mca.lan`).

## Who owns what

- **dash owns:** the app, `ecosystem.config.js`, the `.env` contents, declaring its write-roots + system binaries.
- **Duck Ops owns:** `/srv` provisioning, the deploy rail (`ship`), the box surface (PM2, ingress, OS).
